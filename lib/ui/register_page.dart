...
class _RegisterPageState extends State<RegisterPage> {
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => BlocProvider(
            create: (context) => CounterBloc(feedback, DateFormat('yyyy-MM-dd').format(DateTime.now()), isNewUser),
            child: CounterPage(),
      ),
    ));
  }
  ...
  void finishOnPressed(){
    ...
    if (widget._reg)
      context.read<RegBloc>().add(SignUp(
          loginController.text,
          passwordController.text,
          passwordCheckController.text));
    else
      context.read<RegBloc>().add(SignIn(
  ...
  Widget build(BuildContext context) {
    return BlocListener<RegBloc, RegState>(
      listener: (context, state) {
        if (state.exceptionMessage.isEmpty) openPage(state.feedback, _isNewUser);
              ...
              SingleChildScrollView(
                child: Column(
                  children: [
                    ...
                    TextFieldWithBox(RegisterStrings.password, passwordController),
                    SizedBox(
                      height: 30,
                    ),
                    Visibility(
                      ...
                      child: Column(children: [
                        TextFieldWithBox(RegisterStrings.applyPassword, passwordCheckController),
                      ]),
                    ),
                    SizedBox(
                      height: 30,
